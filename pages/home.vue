...
onMounted(() => {
  ...
  const posts = [
    { lat: 31.287, lng: 121.215, title: '帖子1', content: '这是第一个帖子' },
    { lat: 31.290, lng: 121.220, title: '帖子2', content: '这是第二个帖子' }
  ];
  ...
  window.initBaiduMap = () => {
    ...
    // 遍历所有帖子，添加标记到地图
    posts.forEach(post => {
      const postPoint = new BMap.Point(post.lng, post.lat);
      ...
      marker.addEventListener('click', () => {
        router.push({ name: 'post-detail', query: { lat: post.lat, lng: post.lng } });
      });
    });
